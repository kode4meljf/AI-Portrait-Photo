...
Page({
  ...
  onLoad(options) {
    ...
    let styles = styleIds
      .map((id) => STYLE_LIST.find((s) => s.id === id))
      .filter(Boolean);
    if (!styles.length) {
      styles = STYLE_LIST.slice(0, count);
    }

    this.setData({ originalUrl, styles });
  ...
  computePreviewTarget(rect) {
    ...
    const maxW = ww * 0.92;
    const maxH = (wh - bottomReserve) * 0.88;
    ...
    const left = (ww - w) / 2;
    const top = (wh - bottomReserve - h) / 2;
  ...
  onMakeFrame() {
    ...
    const photoUrl = encodeURIComponent(item.url);
    const styleName = encodeURIComponent(item.name || '');
    wx.navigateTo({
      url: `/pages/cloud/frame-selector/frame-selector?photoUrl=${photoUrl}&styleId=${item.id || ''}&styleName=${styleName}`
    });
  },
