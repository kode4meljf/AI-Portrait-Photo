...
Page({
  ...
  isStoreReady() {
    const id = app.globalData.storeId;
    return id && id !== "mock_store_id";
  },
  ...
  async chooseAvatar() {
    ...
    try {
      ...
      const cloudPath = `store/${app.globalData.storeId}_${Date.now()}.jpg`;
      ...
      await wx.cloud
        .database()
        .collection("store_profile")
        .doc(app.globalData.storeId)
        .update({
          data: { avatarUrl }
        });
