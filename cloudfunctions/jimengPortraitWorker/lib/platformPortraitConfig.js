...
async function readPortraitSettingsFromDatabase() {
  ...
  return {
    ...
    seedreamModelId: normalizeSeedreamModelId(data.seedreamModelId),
...
function invalidatePortraitConfigCache() {
  cache = null;
}
...
async function getSeedreamConfig(options = {}) {
  const config = await getPortraitPlatformConfig(options);
  return {
    modelId: config.seedreamModelId,
    outputSize: resolveSeedreamOutputSize(config.seedreamSizeTier, config.seedreamOrientation)
  };
}
